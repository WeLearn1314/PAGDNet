...
parser.add_argument('--save_every', default=1, type=int, help='save model at every x epoches') #every x epoches save model
    ...
    while(True):
        for i in range(0,len(y_),batch_size):
            ...
            for j in range(i,min(i+batch_size,len(y_))): #read batchsize images, which uses to train
                img = np.array(Image.open(y_[j]).convert('L'),dtype='uint8') #dtype='float32')/255.0#.convert('L')
                img1.append(img)
            get_batch_y = img1
...
def test(model):
    ...
    psnr = []
    ssim = []
    file_list = glob.glob('{}/*.png'.format(args.test_dir)) #notice: it is easy to generate error
